var forbidVideo = function(videoId) {
	...
	$.ajax({
    	url: $("#hdnContextPath").val() + "/video/deleteVideo.action?videoId=" + videoId,
    	type: "POST",
    	async: false,
...
var UsersReportsList = function () {
	...
	var handleUsersReportsList = function() {
        ...
        jqGrid.jqGrid({  
            caption: "视频列表",  
            url: hdnContextPath + "/video/videosList.action",  
            mtype: "post",  
            styleUI: 'Bootstrap',//设置jqgrid的全局样式为bootstrap样式  
            datatype: "json",  
            colNames: ['ID',  '发表者', '视频描述', '视频链接', '点赞数','视频状态', '举报日期', "操作"],  
            colModel: [  
                ...
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                		var button = '<button class="btn btn-outline blue-chambray" id="" onclick=forbidVideo("' + rowObject.dealVideoId + '") style="padding: 1px 3px 1px 3px;">删除</button>';
			    		return button;
			    	}
			    }
            ],  
